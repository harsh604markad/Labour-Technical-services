Imports System.Data
Imports System.Data.OleDb

Public Class Form5
    Dim con As New OleDb.OleDbConnection
    Dim da As New OleDb.OleDbDataAdapter
    Dim ds As New DataSet
    Dim cmd As New OleDb.OleDbCommand
    Dim q As String
    Dim cr As Integer
    Dim bs As New BindingSource


    Private Sub Form5_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        con.ConnectionString = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=D:\Employee.mdb"
        con.Open()
        q = "select * from Bill1"
        da = New OleDb.OleDbDataAdapter(q, con)
        da.Fill(ds, "Bill1")
        DataGridView1.DataSource = ds.Tables("q")
        MsgBox("Connected")


    End Sub

    Private Sub GunaAdvenceButton1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles GunaAdvenceButton1.Click
        q = "insert into Bill1 values(" & CInt(GunaTextBox1.Text) & ",'" & GunaTextBox2.Text & "'," & CInt(GunaTextBox3.Text) & "," & CInt(GunaTextBox4.Text) & "," & CInt(GunaTextBox5.Text) & ")"
        Dim x As Integer
        cmd = New OleDb.OleDbCommand(q, con)
        x = cmd.ExecuteNonQuery()
        If (x) Then
            MsgBox("data inserted")
        Else
            MsgBox("not inserted")
        End If
        ds.Tables.Clear()
        q = "select * from Bill1"
        da = New OleDb.OleDbDataAdapter(q, con)
        da.Fill(ds, "Bill1")
        'GunaDataGridVie.DataSource = ds.Tables("Details1")
    End Sub

    Private Sub GunaButton1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles GunaButton1.Click
        Dim a As Integer
        a = 0

        a = InputBox("enter salary")
        GunaTextBox5.Text = GunaTextBox5.Text - a

        Form3.GunaTextBox1.Text = a

        Form3.Show()




    End Sub

    Private Sub GunaAdvenceButton2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles GunaAdvenceButton2.Click

    End Sub

    Private Sub GunaButton2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles GunaButton2.Click
        bill_print.GunaTextBox3.Text = GunaTextBox5.Text
        bill_print.GunaTextBox1.Text = GunaTextBox1.Text
        bill_print.GunaTextBox1.Text = GunaTextBox1.Text
        bill_print.Show()



    End Sub
End Class